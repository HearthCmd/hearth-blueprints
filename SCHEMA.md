...
## 8. What the release publishes

`index.json`, generated by `scripts/build-index.py` and attached to each release.
Never committed — a committed index drifts from the files it describes; a
generated one cannot.

Each blueprint entry carries its version, a one-line summary for a list row, the
sha256 of each published file, **and its whole parsed body as JSON**.

That last part exists because the consumer which has to *act* on a blueprint —
resolving `@{alias}` and `{{parameter}}` against a real household — speaks JSON
and has no YAML parser. Deriving the JSON here, at release time, keeps exactly
one source of truth (the `.yaml` file you edit) with nothing to keep in sync by
hand.

Practical consequence for authors: **a blueprint only reaches households when a
release is cut.** Merging to `main` is not publishing.
